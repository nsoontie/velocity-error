...
src_dir='../data/bioSVPs-CIOPSEv2/'
pickle_file = os.path.join(src_dir, 'all_bioSVPs-CIOPSEv2.pickle')
...
def main():
    ...
    mesh = plot_error_map(dfall, ax)
    cbar = plt.colorbar(mesh, ax=ax)
    cbar.set_label('ocean speed error [m/s]')
    plt.show()

    
def plot_error_map(df, ax, error='speed_ocean_error',
                   lon_min=-72, lat_min=37, lon_max=-36, lat_max=46,
                   cmap='RdBu_r', vmin=-1, vmax=1, latstep=2, lonstep=5):
